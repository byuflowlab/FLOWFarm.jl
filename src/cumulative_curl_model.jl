function turbine_velocities_one_direction_CC!(turbine_x::Vector{T0}, turbine_y::Vector{T1}, turbine_z::Vector{T2}, rotor_diameter::Vector{T3}, hub_height::Vector{T4}, turbine_yaw::Vector{T5},
    ...
    zPos = turbine_z .+ hub_height

    # loop over all turbines (n)
    @inbounds @fastmath for n=1:n_turbines
        ...
        for d = n+1:n_turbines
            ...
            for p = 1:n_rotor_sample_points
                ...
                if p == 1
                    if no_yaw == false
                        deflections[current_turbine_id,downwind_turbine_id] = wake_deflection_model(x, y, z, turbine_x, turbine_yaw, turbine_ct,
                ...
                calc = max(0.0,a2 - (m*turbine_ct[current_turbine_id]*cos(turbine_yaw[current_turbine_id]))/(16.0*gamma(2/m)*(sigma_n^(2/m))*(1-sum_C/U_inf)^2))
                C_point = (1-sum_C/U_inf) * (a1-sqrt(calc))
                r_tilde = sqrt((y-y_n-dy)^2 + (z-z_n)^2)/rotor_diameter[current_turbine_id]
                velDef = C_point*exp(-1 * (r_tilde^m)/(2.0*sigma_n))
...
function wake_expansion(Ct,TI,x_tilde,model)
    ...
    @fastmath epsilon = (model.c_s1*Ct+model.c_s2)*sqrt(beta)
    @fastmath k = model.a_s*TI+model.b_s
    @fastmath sigma = k*x_tilde+epsilon
    return @fastmath sigma^2
end