...
for i in 1:length(off)
    ...
    #
    # waked_damage = ff.get_damage(turbine_x,turbine_y,turb_index,speeds,omegas,pitches,wind_speed,Rhub,Rtip,r,chord,theta,airfoils,hubHt,ms,pd,wind_speed,Nlocs=20,shearExp=shearexponent,mult=mult,m2=m2)

    # find velocities
    # find loads
    # find fatigue

    offset = off[i]*rotor_diameter
    ...
    # Omega_rpm = omega_func(ws) #in rpm
